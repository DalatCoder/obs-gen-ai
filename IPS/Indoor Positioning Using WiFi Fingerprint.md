...
- **Từ khóa:** Indoor Positioning System (Hệ thống định vị trong nhà), WiFi, Fingerprinting (Vân tay), Stacked Autoencoder (Bộ tự mã hóa xếp chồng), Deep Neural Network (Mạng nơ-ron sâu) 6
    

# Mục tiêu nghiên cứu

Mục tiêu chính của bài báo là đề xuất một kỹ thuật định vị trong nhà bằng cách sử dụng phương pháp

**vân tay WiFi (WiFi fingerprinting)**. 7Cụ thể, nghiên cứu tập trung vào việc áp dụng một

**mạng nơ-ron sâu (Deep Neural Network - DNN)** với kiến trúc **bộ tự mã hóa xếp chồng (Stacked Autoencoder - SAE)** để xác định vị trí tương đối của người dùng, bao gồm họ đang ở **tòa nhà nào** và **tầng nào**. 8

# Bối cảnh và động lực

- **Vấn đề:** Hệ thống Định vị Toàn cầu (Global Positioning System - GPS) hoạt động rất hiệu quả cho việc định vị ngoài trời nhưng lại mất tín hiệu khi ở trong các tòa nhà. 9999Do đó, định vị trong nhà (Indoor Positioning) vẫn là một bài toán đầy thách thức. 10
...
- **Hệ thống Định vị trong nhà (Indoor Positioning System - IPS):** Là hệ thống giúp xác định vị trí của người hoặc vật thể bên trong các công trình xây dựng bằng cách sử dụng các tín hiệu vô tuyến, từ trường hoặc các loại cảm biến khác. 15151515
    
- **Vân tay WiFi (WiFi Fingerprinting):** Là một phương pháp định vị phổ biến, bao gồm hai giai đoạn:
    
    1. **Giai đoạn Ngoại tuyến (Offline phase):** Thu thập và ghi lại **Cường độ Tín hiệu Thu được (Received Signal Strength - RSS)** từ các điểm truy cập WiFi (Access Points) tại nhiều vị trí đã biết trước tọa độ. Dữ liệu này được lưu trữ trong một cơ sở dữ liệu, tạo thành một "bản đồ vân tay" tín hiệu. 161616
        
    2. **Giai đoạn Trực tuyến (Online phase):** Thiết bị của người dùng quét mẫu RSS tại vị trí hiện tại, sau đó tìm kiếm trong cơ sở dữ liệu để tìm ra mẫu "vân tay" tương đồng nhất và từ đó suy ra vị trí của người dùng. 17171717
        
- **Cường độ Tín hiệu Thu được (Received Signal Strength - RSSI):** Là thước đo cường độ của tín hiệu WiFi mà thiết bị nhận được từ một điểm truy cập. Trong nghiên cứu này, giá trị RSSI là các số âm, tính bằng dBm, dao động từ -104dBm (tín hiệu rất yếu) đến 0dBm (tín hiệu rất mạnh). 18181818Giá trị dương 100 được dùng để biểu thị rằng điểm truy cập đó không được phát hiện. 19191919
    
- **Bộ tự mã hóa xếp chồng (Stacked Autoencoder - SAE):** Là một mạng nơ-ron được tạo thành từ nhiều lớp **bộ tự mã hóa (Autoencoder)**. Nó được sử dụng để học các đặc trưng (features) của dữ liệu đầu vào một cách không giám sát và giảm chiều dữ liệu. 20202020Trong bài báo, SAE được dùng để nén dữ liệu RSSI từ 520 chiều xuống một không gian nhỏ hơn. 2121
    
- **Mạng nơ-ron sâu (Deep Neural Network - DNN):** Là mạng nơ-ron nhân tạo có nhiều lớp ẩn (hidden layers) nằm giữa lớp đầu vào (input layer) và lớp đầu ra (output layer). 22
    

# Phương pháp và kỹ thuật

Hệ thống được xây dựng dựa trên phương pháp vân tay WiFi và mô hình học sâu.

### 1. Kiến trúc tổng thể

Hệ thống bao gồm các bước:

1. **Thu thập tín hiệu:** Thiết bị người dùng quét và ghi nhận tín hiệu WiFi từ các điểm truy cập (Access Points) xung quanh. 2323
    
2. **Xây dựng cơ sở dữ liệu vân tay:** Ở giai đoạn ngoại tuyến, cường độ tín hiệu (RSSI) tại các vị trí cụ thể được đo đạc và lưu vào cơ sở dữ liệu cùng với thông tin vị trí (tòa nhà, tầng). 24242424
    
3. **Xác định vị trí:** Ở giai đoạn trực tuyến, mô hình sẽ nhận dữ liệu RSSI hiện tại và dự đoán vị trí dựa trên cơ sở dữ liệu đã huấn luyện. 25
    

### 2. Mô hình học máy

Mô hình chính là một **Mạng Nơ-ron Sâu (DNN)** sử dụng **Bộ tự mã hóa xếp chồng (SAE)**, được huấn luyện qua hai giai đoạn:
...
- **Giai đoạn 1: Tiền huấn luyện không giám sát (Unsupervised Pre-training)**
    ...
    - **Kiến trúc:** Một bộ tự mã hóa (Autoencoder) được sử dụng.
        
        - **Đầu vào (Encoder Input):** Vector 520 giá trị RSSI từ các điểm truy cập WiFi. 26262626
...
- **Thuộc tính:** Mỗi mẫu có 529 thuộc tính: 35
    ...
    - **520 thuộc tính đầu tiên:** Là giá trị RSSI của 520 điểm truy cập WiFi (WAP). 36
...
- **Hiệu quả:** Mô hình DNN cho thấy hiệu suất cao trong việc xác định đúng tòa nhà và tầng, với độ chính xác khoảng 93%. 42
...
- **Thách thức:** Định vị trong nhà là một bài toán phức tạp và chưa có giải pháp toàn cầu nào phù hợp cho mọi ứng dụng. 45
...
Đây là một bài báo trình bày một cách tiếp cận chuẩn mực và hiệu quả cho bài toán định vị trong nhà. Việc kết hợp phương pháp **vân tay WiFi** cổ điển với kỹ thuật **học sâu (Deep Learning)** hiện đại là điểm nhấn chính, cho thấy tiềm năng lớn của AI trong việc giải quyết các vấn đề kỹ thuật thực tiễn.
